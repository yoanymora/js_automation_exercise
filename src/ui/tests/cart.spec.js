...
describe("Cart", () => {
	...
	it("User adds a product to the cart with chai", async () => {
		...
		await HomePage.productCards[0].click();
		await ProductDetailsPage.addToCartButton.click();
		await Common.waitUntilUpdateText(
			await ProductDetailsPage.confirmationMessageContainer,
			CartPage.productAddedToCartMessage
		);
		chaiExpect(
			await Common.getSelectorText(await ProductDetailsPage.confirmationMessageContainer)
		).to.equal(CartPage.productAddedToCartMessage);
		chaiExpect(
			await Common.getSelectorText(await ProductDetailsPage.header.cartQuantity)
		).to.equal("3");
	});
	...
	it("User increases the quantity of products in its cart with chai", async () => {
		...
		chaiExpect(await Common.getSelectorText(await CartPage.successMessage)).to.equal(
			CartPage.productQuantityUpdatedMessage
		);
		chaiExpect(await Common.getSelectorText(await CartPage.linePrice)).to.equal(
			CartPage.productPriceValue
		);
		chaiExpect(await Common.getSelectorText(await CartPage.cartTotal)).to.equal(
			CartPage.productPriceValue
		);
	});
});
