...
describe('Cart', () => {
    ...
    it('User adds a product to the cart with chai', async () => {
        await HomePage.open();
        await Common.waitForClickable(await HomePage.productCards[0], 5000);
        await HomePage.productCards[0].click();
        await Common.waitForClickable(await ProductDetailsPage.addToCartButton, 5000);
        await ProductDetailsPage.addToCartButton.click();
        await Common.waitForVisible(await ProductDetailsPage.confirmationMessageContainer, 5000);
        ...
        await Common.waitForVisible(await ProductDetailsPage.header.cartQuantity, 5000);
    ...
    it('User goes to the cart page with chai', async() => {
        ...
        chaiExpect(await Common.getSelectorText(await ProductDetailsPage.header.cartQuantity)).to.equal('3');
        await CartPage.open();
        await Common.waitForExisting(await CartPage.productsTable);
    ...
    it('User increases the quantity of products in its cart with chai', async () => {
        ...
        chaiExpect(await Common.getSelectorText(await ProductDetailsPage.header.cartQuantity)).to.equal('3');
        await CartPage.open();
        await Common.waitForExisting(await CartPage.productQuantity);
        await Common.waitForVisible(await CartPage.productQuantity, 5000);
        await CartService.setProductQuantity('2');
