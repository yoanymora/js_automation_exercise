...
class HomeService {
	...
	async waitUntilFilterSortingCompleted(action, timeout = 5000) {
		await browser.waitUntil(
			async () =>
				(await HomePage.productsGrid.getAttribute("data-test")) === `${action}_completed`,
			{ timeout }
		);
	}
	...
	async computePriceRangeTo39() {
		...
		let valueToRest =
			(39 * (await priceSelectorBarWidthValue)) /
			parseInt(await HomePage.filter.priceRangeMaxValue.getText());
